...
const textarea = document.querySelector("#colored_text_editor > textarea") as HTMLTextAreaElement;
const content = document.querySelector("#colored_text_editor > div") as HTMLDivElement;
if (textarea === undefined) throw Error("Textarea not found");

namespace Highlighting {
    function __(color: string, txt: string) {
    ...
    //     ["\"", "'", "`"].forEach(s => text = text.replace(eval(`/(?<!\\\\)(${s})(.+?)(?<!\\\\)(\\1)/gs`), __(HIGHLIGHTING.js.string, "$1$2$3")))
    // return text;
    // }
    ...
    // export function highlighting(lines: Set<Line>) {
...
namespace Textarea {
    export function rewrite(text: string) {
        ...
        // const lines: Set<Line> = new Set(text.split("\n").map(t => new Line(t)));
        // Highlighting.highlighting(lines);
        // text = Highlighting.string(text);
        // text = Highlighting.keywords(text);
        // text = Highlighting.variables(text);
